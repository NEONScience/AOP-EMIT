...
# Updated by Bridget Hass to include a 2nd calc_ewt function that works on NEON relfectance datasets.
...
# Modified from calc_ewt to work on NEON reflectance datasets, also saved as netcdf files
# main differences are removing the metadata fields that don't exist in the NEON data
def calc_ewt_neon(
    filepath: str,
    outdir: str,
    n_cpu: int = (cpu_count() - 1),
    ...
    """
    # Get number of rows to break up in parallel
    neon_ds = xr.open_dataset(filepath, decode_coords="all")
    ...
    # Check if wavelengths are set as a dimension, if not throw a warning message
    if "wavelengths" not in neon_ds.dims:
        print('Warning! "wavelengths" are not included in the dataset dimensions.')

    # Get number of rows
    n_rows = neon_ds["reflectance"].shape[0]

    # Get Wavelength centers - convert to float64
    # scipy.optimize.least_squares function is extremely sensitive to the dtypes of objective function arguments
    # Here float64 performs better: less iterations, lower value of the cost function.
    # Also, jacobian estimates get more accurate and lead to better convergence w/ higher precision
    wvl = np.float64(neon_ds["wavelengths"].data)
    ...
    # Create Data Vars - Squeeze bands dim
    data_vars = {"cwc": (list(neon_ds.dims.keys())[0:2], cwc.squeeze())}
    ...
    try:
        ...
        print('Error! Could not create the COG file.')
        print(e)
