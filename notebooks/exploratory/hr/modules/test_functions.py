...
def data_download_tracker(absolute_soap_path, folder_names, extension):
    """
    Explores 'SOAP' directory to confirm data downloaded into correct
    folders and finds files with a specific extension within those
    folders and their subdirectories.

    Args:
        absolute_soap_path (str): The absolute path to the root of
                                  your project.
        folder_names (list): A list of subfolders within 'SOAP' to
                             explore (e.g., ["EMIT", "NEON"]).
        extension (str): The file extension to search for
                         (".nc", ".shp", ".CHM", ".H5").

    Returns:
        dict: A dictionary where keys are the folder names
              (e.g., "EMIT", "NEON") and values are lists
              of absolute paths to the found files within that folder.
    """
    ...
    print(f"\n--- Exploring within the SOAP directory: {absolute_soap_path} ---")

    for folder_name in folder_names:
        current_data_folder = os.path.join(absolute_soap_path, folder_name)
        if not os.path.isdir(current_data_folder):
            print(f"Warning: Folder '{folder_name}' doesn't exist at:"
                  f"{current_data_folder}")
            all_found_files[folder_name] = [] # Record empty list for this folder
            continue
            
        print(f"\n--- Finding '{extension}' files in: {current_data_folder} ---")
        ...
        if found_files_in_current_folder:
            print(f"Found {len(found_files_in_current_folder)} '{extension}' "
                  f"files in '{folder_name}'.")
        else:
...
def surfrfl_hvplot_image(ds,
                         ...
                         # This would let a user bypass our fxn and
                         # Add other .hvplot.image parameters
                         **kwargs):
    """
    ...
    **kwargs
    ...
    Retrieved June 28, 2025,
    from https://hvplot.holoviz.org/reference/xarray/image.html
    .. [2] Plotting Options—hvPlot 0.11.3 documentation. (n.d.).
    Retrieved June 28, 2025,
    from https://hvplot.holoviz.org/ref/plotting_options/index.html
    """
    plot = (
        ds
        .hvplot
